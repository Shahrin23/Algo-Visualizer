...
class GraphVisualizer: public QWidget
{
    ...
    struct Node {
        ...
        int parentId = -1;
        QString label;// Parent node in the shortest path
    };
...
public:
    ...
    void setNumberOfNodesAStar(int count);   // places nodes by coordinate pairs
    void aStarAlgorithm(int srcId, int destId);
    void placeAStarNode(const QPoint &clickpos, const QString &label);
    int findNodeByCoord(const QPoint &pos) const;
    void clearCanvas();
    QString getStatusAt(int index) const;

signals:
    void requestEdgeValueInput(const QPoint &pos, int edgeIndex);
    void requestAStarNodeInput(const QPoint &clickPos);
...
private:
    ...
    struct AStarNode {
        int     id;       // corresponds to GraphVisualizer::Node::id
        float   g;        // actual cost from source
        float   h;        // heuristic to goal
        int     parentId; // id of the came-from node

        float f() const { return g + h; }

        // For std::priority_queue with greater<AStarNode> → min-heap on f()
