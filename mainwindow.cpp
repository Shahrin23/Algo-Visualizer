    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(graphVisualizer, &GraphVisualizer::requestAStarNodeInput,this, &MainWindow::handleAStarNodeInput);
    connect(ui->AlgoComboBox_2, QOverload<int>::of(&QComboBox::currentIndexChanged),this, &MainWindow::onAlgoComboChanged);
    //connect(graphVisualizer, &GraphVisualizer::statusUpdated,this, &MainWindow::onStatusUpdated);
...
void MainWindow::onAlgoComboChanged(int index)
{
    graphVisualizer->clearCanvas();
}
...
void MainWindow::promptForNumberOfNodes()
{
    ...
    if (ui->AlgoComboBox_2->currentIndex() == 3) {
        graphVisualizer->setNumberOfNodesAStar(nodeCount); // just sets mode + count
    } else {
...
void MainWindow::handleAStarNodeInput(const QPoint &clickPos)
{
    bool ok;
    QString text = QInputDialog::getText(
        this,
        tr("Label Node"),
        tr("Enter x y label for this node (e.g. '3 5'):"),
        QLineEdit::Normal, "", &ok);
    ...
    if (parts.size() != 2) {
        QMessageBox::warning(this, tr("Input Error"), tr("Enter exactly two values: x y"));
        return;
    }
    ...
    if (!okX || !okY) {
        QMessageBox::warning(this, tr("Input Error"), tr("Values must be integers."));
        return;
    }

    QString label = QString("%1,%2").arg(x).arg(y);

    // Check duplicate label
    if (graphVisualizer->findNodeByCoord(QPoint(x, y)) != -1) {
        QMessageBox::warning(this, tr("Input Error"),
                             tr("A node with label (%1,%2) already exists.").arg(x).arg(y));
        return;
    }

    // clickPos = where user clicked on canvas (circle drawn here)
    // label    = what user typed (shown inside the circle)
    graphVisualizer->placeAStarNode(clickPos, label);
}
...
void MainWindow::promptForAStarSrcDest()
{
    // Helper lambda: asks for a coordinate pair and validates against placed nodes
    // Returns the matching node's internal ID, or -1 on cancel
    auto askCoord = [&](const QString &title) -> int {
        while (true) {
            bool ok;
            QString text = QInputDialog::getText(
                this, title,
                tr("Enter x y (e.g. '100 200') — must match a placed node:"),
                QLineEdit::Normal, "", &ok);

            if (!ok) return -1;  // user cancelled

            QStringList parts = text.split(QRegularExpression("[ ,]+"), Qt::SkipEmptyParts);
            if (parts.size() != 2) {
                QMessageBox::warning(this, tr("Input Error"),
                                     tr("Please enter exactly two values: x y"));
                continue;
            }
            ...
            // Find matching node
            int matchId = graphVisualizer->findNodeByCoord(QPoint(x, y));
            if (matchId == -1) {
                QMessageBox::warning(this, tr("No Match"),
                                     tr("No node found at (%1, %2). Please re-enter.").arg(x).arg(y));
    ...
    int srcId  = askCoord(tr("A* — Source Node"));
    if (srcId == -1) return;

    int destId = askCoord(tr("A* — Destination Node"));
    if (destId == -1) return;

    if (srcId == destId) {
        QMessageBox::warning(this, tr("Invalid"),
                             tr("Source and destination cannot be the same node."));
        return;
    }

    int interval = ui->spinBox_2->value();
    graphVisualizer->startAlgorithm(3, srcId, destId, interval);
}
