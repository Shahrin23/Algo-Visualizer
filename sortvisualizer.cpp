...
void SortVisualizer::paintEvent(QPaintEvent *event) {
    ...
    font.setPixelSize(qMin(30,qMax(30, rectangleWidth / 4))); // Adjust font size based on bar width
    painter.setFont(font);
    // painter2.setPen(Qt::white);
    // painter2.drawText(rect(), Qt::AlignTop | Qt::AlignHCenter, currentStatus);
    for (int i = 0; i < numberOfElements; i++) {
        ...
        if(rectanglesArray[i].y()<30){
            painter.setPen(Qt::white);
            int textPadding = 30;
            QRect textRect(i * rectangleWidth,rectanglesArray[i].y() + textPadding, rectangleWidth, 33);
            painter.drawText(textRect,Qt::AlignBottom | Qt::AlignHCenter , QString::number(numbersArray[i]));
            painter.setPen(rectangleWidth >= 3 ? colors[DefaultBorderColor] : Qt::NoPen);
        }else{
            painter.setPen(Qt::white);
            int textPadding = 30;
            QRect textRect(i * rectangleWidth,rectanglesArray[i].y() - textPadding, rectangleWidth, 37);
            painter.drawText(textRect,Qt::AlignBottom | Qt::AlignHCenter , QString::number(numbersArray[i]));
            painter.setPen(rectangleWidth >= 3 ? colors[DefaultBorderColor] : Qt::NoPen);
        }
    }
    if (!currentStatus.isEmpty()) {
        ...
        // 1. Define the Pop-Screen dimensions
        int bannerWidth = 300;
        int bannerHeight = 50;
        // Position it in the top-center
        QRect bannerRect(20, 20, bannerWidth, bannerHeight);

        // 2. Draw the background (Semi-transparent dark grey)
        painter.setPen(Qt::NoPen);
        painter.setBrush(QColor(0, 0, 0, 80));// 180 is the transparency (0-255)
        painter.drawRoundedRect(bannerRect, 15, 15);// Rounded corners

        // 3. Draw a border (Accent color)
        QPen borderPen(Qt::white); // Cyan border
        borderPen.setWidth(2);
        painter.setPen(borderPen);
        painter.setBrush(Qt::NoBrush);
        painter.drawRoundedRect(bannerRect, 15, 15);
        ...
        painter.drawText(bannerRect, Qt::AlignCenter, currentStatus);
    }
...
void SortVisualizer::setCustomHeights(const QVector<int> &heights)
{
    stepsVector.clear();
    currIndex = 0;
    numbersArray = heights;
    rectangleColors.clear();
    for (int i = 0; i < heights.size(); i++) {
        rectangleColors.append(DefaultFillColor);
    }
    numberOfElements = numbersArray.size();
    rectanglesArray.clear();
    if (numberOfElements > 0 && width() > 0) {
        rectangleWidth = width() / numberOfElements;
    } else {
        rectangleWidth = 0;
    }
    for (int i = 0; i < numberOfElements; i++) {
    ...
    rectangleColors.fill(Colors::DefaultFillColor, numberOfElements);
    repaint();
    emit enableSortButtons();
    update();
}
...
void SortVisualizer::processCurrentStep() {
    ...
    case HIGHLIGHT:
        currentStatus = QString("Comparing indices %1 and %2").arg(i).arg(j);
        processHighlight(i, j);
        break;
    case SWAP:
        currentStatus = QString("Swapping %1 and %2").arg(numbersArray[i]).arg(numbersArray[j]);
        processSwap(i, j);
        break;
    case SET_HEIGHT:
        currentStatus = QString("Setting index %1 to height %2").arg(i).arg(j);
...
void SortVisualizer::processSwapHighlight(int i, int j) {
    //qDebug() << i << " " << j;
    rectangleColors[i] = HighlightSwapColor;
    rectangleColors[j] = HighlightSwapColor;
}
void SortVisualizer::processSwap(int i, int j) {
    processSwapHighlight(i, j);
    swapRectangles(i, j);
}
